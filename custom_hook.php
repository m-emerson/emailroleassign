...
function saml_hook_authorize_user($username, $saml_attributes, $authorize_user) {
    /* Set the saml attributes to a session variable so we can access them on user creation */
    if ($authorize_user)
...
function saml_hook_post_user_created($user) {
    /* Assign the correct role if the email is within the allowed emails list */
    $saml_roles = $_SESSION['saml_attributes']['roles'];
    global $DB;

    $role_mapping = array();	

    /* See if any of the saml roles for this user map to any moodle role short names */
    for ($i = 0; $i < count($saml_roles); $i++) {
        if ( $mdl_role = $DB->get_record('role', array('shortname'=>$saml_roles[$i])) ) {
            array_push($role_mapping, $mdl_role);
        }
    }

    /* Map roles if we found any */
    for ($i = 0; $i < count($role_mapping); $i++) {
        role_assign($role_mapping[$i]->id, $user->id, $context->id);	
    }
}
